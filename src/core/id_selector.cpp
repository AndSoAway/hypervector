...
bool IDSelectorRange::IsMember(idx_t id) const {
  return id >= imin && id < imax;
}

void IDSelectorRange::FindSortedIdsBounds(size_t list_size, const idx_t* ids,
                                             size_t* jmin_out,
                                             size_t* jmax_out) const {
  HYPERVEC_ASSERT(assume_sorted);
...
bool IDSelectorArray::IsMember(idx_t id) const {
  for (idx_t i = 0; i < n; i++) {
    if (ids[i] == id) {
      return true;
    }
...
bool IDSelectorBatch::IsMember(idx_t i) const {
  long im = i & mask;
...
bool IDSelectorBitmap::IsMember(idx_t ii) const {
  uint64_t i = ii;
  if ((i >> 3) >= n) {
    return false;
  }
