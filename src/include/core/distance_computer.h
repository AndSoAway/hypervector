...
struct DistanceComputer {
  ...
  virtual void SetQuery(const float* x) = 0;
  ...
  /// heavily from this.
  virtual void distances_batch_4(const idx_t idx0, const idx_t idx1,
...
struct FlatCodesDistanceComputer : DistanceComputer {
  ...
  virtual float partial_dot_product(const idx_t /* i */,
                                    const uint32_t /* offset */,
                                    const uint32_t /* num_components */) {
    HYPERVEC_THROW_MSG("partial_dot_product not implemented");
  }
  ...
  virtual void partial_dot_product_batch_4(const idx_t idx0, const idx_t idx1,
                                           const idx_t idx2, const idx_t idx3,
                                           float& dp0, float& dp1, float& dp2,
                                           float& dp3, const uint32_t offset,
                                           const uint32_t num_components) {
    // default implementation for correctness
    const float d0 = this->partial_dot_product(idx0, offset, num_components);
    const float d1 = this->partial_dot_product(idx1, offset, num_components);
    const float d2 = this->partial_dot_product(idx2, offset, num_components);
    const float d3 = this->partial_dot_product(idx3, offset, num_components);
    dp0 = d0;
    dp1 = d1;
    dp2 = d2;
    dp3 = d3;
