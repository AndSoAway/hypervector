...
struct IDSelectorRange : IDSelector {
  ...
  bool IsMember(idx_t id) const final;

  /// for sorted ids, find the range of list indices where the valid ids are
  /// stored
  void FindSortedIdsBounds(size_t list_size, const idx_t* ids, size_t* jmin,
                              size_t* jmax) const;
 ...
 *
 * IsMember calls are very inefficient, but some operations can use the ids
 * directly.
 */
...
struct IDSelectorAnd : IDSelector {
  ...
  IDSelectorAnd(const IDSelector* lhs, const IDSelector* rhs)
    : lhs(lhs), rhs(rhs) {}
  bool IsMember(idx_t id) const final {
    return lhs->IsMember(id) && rhs->IsMember(id);
  }
