...
struct MaybeOwnedVector {
  ...
  explicit MaybeOwnedVector(const std::vector<T>& vec)
    : hypervec::MaybeOwnedVector<T>(vec.size()) {
    if (vec.size() > 0) {
  ...
  static MaybeOwnedVector create_view(
    void* address, const size_t n_elements,
  ...
  T& at(size_type pos) {
    HYPERVEC_ASSERT_MSG(
      is_owned, "This operation cannot be performed on a viewed vector");
...
template <typename T>
bool operator==(const MaybeOwnedVector<T>& lhs,
