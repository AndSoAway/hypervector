...
#ifdef HYPERVEC_MAIN_LIB
#define HYPERVEC_API __declspec(dllexport)
#else  // _HYPERVEC_MAIN_LIB
#define HYPERVEC_API __declspec(dllimport)
#endif  // HYPERVEC_MAIN_LIB
...
// On non-Windows, HYPERVEC_PACKED handles packing, so these are no-ops
#define HYPERVEC_PACK_STRUCTS_BEGIN
#define HYPERVEC_PACK_STRUCTS_END

#define HYPERVEC_ALWAYS_INLINE __attribute__((always_inline)) inline

#endif

#if defined(__GNUC__) || defined(__clang__)
#define HYPERVEC_DEPRECATED(msg) __attribute__((deprecated(msg)))
#else
#define HYPERVEC_DEPRECATED(msg)
#endif  // GCC or Clang

// Localized enablement of imprecise floating point operations
// You need to use all 3 macros to cover all compilers.
#if defined(_MSC_VER)
#define HYPERVEC_PRAGMA_IMPRECISE_LOOP
#define HYPERVEC_PRAGMA_IMPRECISE_FUNCTION_BEGIN \
  __pragma(float_control(precise, off, push))
#define HYPERVEC_PRAGMA_IMPRECISE_FUNCTION_END __pragma(float_control(pop))
...
#define HYPERVEC_PRAGMA_IMPRECISE_FUNCTION_END _Pragma("float_control(pop)")
#else
#define HYPERVEC_PRAGMA_IMPRECISE_LOOP \
...
#else
#define HYPERVEC_PRAGMA_IMPRECISE_FUNCTION_BEGIN
#define HYPERVEC_PRAGMA_IMPRECISE_FUNCTION_END
#endif
#endif
