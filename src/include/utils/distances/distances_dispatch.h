...
inline float fvec_inner_product_dispatch(const float* x, const float* y,
                                         size_t d) {
  DISPATCH_SIMDLevel(fvec_inner_product, x, y, d);
}

inline void fvec_inner_product_batch_4_dispatch(const float* x, const float* y0,
                                                const float* y1,
                                                const float* y2,
                                                const float* y3, const size_t d,
                                                float& dis0, float& dis1,
                                                float& dis2, float& dis3) {
  DISPATCH_SIMDLevel(fvec_inner_product_batch_4, x, y0, y1, y2, y3, d, dis0,
                     dis1, dis2, dis3);
}

inline void fvec_L2sqr_batch_4_dispatch(const float* x, const float* y0,
                                        const float* y1, const float* y2,
                                        const float* y3, const size_t d,
                                        float& dis0, float& dis1, float& dis2,
                                        float& dis3) {
  DISPATCH_SIMDLevel(fvec_L2sqr_batch_4, x, y0, y1, y2, y3, d, dis0, dis1, dis2,
                     dis3);
}

inline void fvec_L2sqr_ny_transposed_dispatch(float* dis, const float* x,
                                              const float* y,
                                              const float* y_sqlen, size_t d,
                                              size_t d_offset, size_t ny) {
  DISPATCH_SIMDLevel(fvec_L2sqr_ny_transposed, dis, x, y, y_sqlen, d, d_offset,
                     ny);
}

inline void fvec_inner_products_ny_dispatch(float* ip, const float* x,
                                            const float* y, size_t d,
                                            size_t ny) {
  DISPATCH_SIMDLevel(fvec_inner_products_ny, ip, x, y, d, ny);
}

inline void fvec_L2sqr_ny_dispatch(float* dis, const float* x, const float* y,
                                   size_t d, size_t ny) {
  DISPATCH_SIMDLevel(fvec_L2sqr_ny, dis, x, y, d, ny);
}

inline size_t fvec_L2sqr_ny_nearest_dispatch(float* distances_tmp_buffer,
                                             const float* x, const float* y,
                                             size_t d, size_t ny) {
  DISPATCH_SIMDLevel(fvec_L2sqr_ny_nearest, distances_tmp_buffer, x, y, d, ny);
}
...
inline size_t fvec_L2sqr_ny_nearest_y_transposed_dispatch(
  ...
  DISPATCH_SIMDLevel(fvec_L2sqr_ny_nearest_y_transposed, distances_tmp_buffer,
                     x, y, y_sqlen, d, d_offset, ny);
}

inline void fvec_madd_dispatch(size_t n, const float* a, float bf,
                               const float* b, float* c) {
  DISPATCH_SIMDLevel(fvec_madd, n, a, bf, b, c);
}

inline int fvec_madd_and_argmin_dispatch(size_t n, const float* a, float bf,
                                         const float* b, float* c) {
  DISPATCH_SIMDLevel(fvec_madd_and_argmin, n, a, bf, b, c);
}

inline void fvec_sub_dispatch(size_t d, const float* a, const float* b,
                              float* c) {
  with_simd_level_256bit(
    [&]<SIMDLevel level>() { fvec_sub<level>(d, a, b, c); });
}

inline void fvec_add_dispatch(size_t d, const float* a, const float* b,
                              float* c) {
  with_simd_level_256bit(
    [&]<SIMDLevel level>() { fvec_add<level>(d, a, b, c); });
}

inline void fvec_add_scalar_dispatch(size_t d, const float* a, float b,
                                     float* c) {
  with_simd_level_256bit(
    [&]<SIMDLevel level>() { fvec_add<level>(d, a, b, c); });
}

inline void compute_PQ_dis_tables_dsub2_dispatch(size_t d, size_t ksub,
                                                 const float* centroids,
                                                 size_t nx, const float* x,
                                                 bool is_inner_product,
                                                 float* dis_tables) {
  with_simd_level_256bit([&]<SIMDLevel level>() {
    compute_PQ_dis_tables_dsub2<level>(d, ksub, centroids, nx, x,
