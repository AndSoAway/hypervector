...
constexpr hypervec::SIMDLevel SL = THE_SIMD_LEVEL;
/******************************************************************
...
float fvec_norm_L2sqr<SL>(const float* x, size_t d) {
  ...
  float res = 0;
  HYPERVEC_PRAGMA_IMPRECISE_LOOP
...
HYPERVEC_PRAGMA_IMPRECISE_FUNCTION_BEGIN
template <>
void fvec_L2sqr_batch_4<SL>(const float* x, const float* y0, const float* y1,
...
template <>
float VectorDistance<kMetricL2, SL>::operator()(const float* x,
                                                const float* y) const {
  return fvec_L2sqr<SL>(x, y, this->d);
}

template <>
float VectorDistance<kMetricInnerProduct, SL>::operator()(
  const float* x, const float* y) const {
  return fvec_inner_product<SL>(x, y, this->d);
}

template <>
float VectorDistance<kMetricL1, SL>::operator()(const float* x,
                                                const float* y) const {
  return fvec_L1<SL>(x, y, this->d);
}

template <>
float VectorDistance<kMetricLinf, SL>::operator()(const float* x,
                                                  const float* y) const {
  return fvec_Linf<SL>(x, y, this->d);
}

template <>
float VectorDistance<kMetricLp, SL>::operator()(const float* x,
                                                const float* y) const {
  float accu = 0;
...
template <>
float VectorDistance<kMetricCanberra, SL>::operator()(const float* x,
                                                      const float* y) const {
  float accu = 0;
  for (size_t i = 0; i < this->d; i++) {
    float xi = x[i], yi = y[i];
...
template <>
float VectorDistance<kMetricBrayCurtis, SL>::operator()(const float* x,
                                                        const float* y) const {
...
template <>
float VectorDistance<kMetricJensenShannon, SL>::operator()(
  const float* x, const float* y) const {
  float accu = 0;
  for (size_t i = 0; i < this->d; i++) {
    float xi = x[i], yi = y[i];
...
template <>
float VectorDistance<kMetricJaccard, SL>::operator()(const float* x,
                                                     const float* y) const {
  float accu_num = 0, accu_den = 0;
  for (size_t i = 0; i < this->d; i++) {
...
template <>
float VectorDistance<kMetricNaNEuclidean, SL>::operator()(
  const float* x, const float* y) const {
  float accu = 0;
  size_t present = 0;
  for (size_t i = 0; i < this->d; i++) {
...
template <>
float VectorDistance<kMetricGower, SL>::operator()(const float* x,
                                                   const float* y) const {
  float accu = 0;
  size_t valid_dims = 0;
