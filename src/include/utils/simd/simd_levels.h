...
/// Convert SIMDLevel to string. Throws HypervecException for invalid level.
std::string to_string(SIMDLevel level);

/// Parse string to SIMDLevel. Throws HypervecException for invalid strings.
 ...
 * regardless of whether hypervec was built with Dynamic Dispatch (DD) or static
 * SIMD selection.
 *
 * In DD mode:
...
struct HYPERVEC_API SIMDConfig {
  ...
  SIMDConfig(const char** hypervec_simd_level_env = nullptr);

  /// Set the SIMD level. Throws HypervecException if level is not supported.
