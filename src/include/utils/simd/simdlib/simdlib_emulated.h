...
struct simd16uint16_tpl<SIMDLevel::NONE> : simd256bit_tpl<SIMDLevel::NONE> {
  ...
  template <typename F>
  static simd16uint16_tpl binary_func(const simd16uint16_tpl& a,
                                      const simd16uint16_tpl& b, F&& f) {
    simd16uint16_tpl c;
    for (int j = 0; j < 16; j++) {
  ...
  simd16uint16_tpl operator&(
    ...
    return binary_func(*this, simd16uint16_tpl(other),
                       [](uint16_t a, uint16_t b) { return a & b; });
  }
  ...
  simd16uint16_tpl operator==(const simd16uint16_tpl& other) const {
    return binary_func(
      *this, other, [](uint16_t a, uint16_t b) { return a == b ? 0xffff : 0; });
  }
...
inline uint32_t cmp_ge32(const simd16uint16_tpl<SIMDLevel::NONE>& d0,
...
inline uint32_t cmp_le32(const simd16uint16_tpl<SIMDLevel::NONE>& d0,
                         const simd16uint16_tpl<SIMDLevel::NONE>& d1,
                         const simd16uint16_tpl<SIMDLevel::NONE>& thr) {
  uint32_t gem = 0;
  for (int j = 0; j < 16; j++) {
...
inline void cmplt_min_max_fast(
  ...
  for (size_t i = 0; i < 16; i++) {
    ...
    minIndices.u16[i] = flag ? candidateIndices.u16[i] : currentIndices.u16[i];
    maxValues.u16[i] = !flag ? candidateValues.u16[i] : currentValues.u16[i];
    maxIndices.u16[i] = !flag ? candidateIndices.u16[i] : currentIndices.u16[i];
  }
}
...
struct simd32uint8_tpl<SIMDLevel::NONE> : simd256bit_tpl<SIMDLevel::NONE> {
  ...
  template <typename F>
  static simd32uint8_tpl binary_func(const simd32uint8_tpl& a,
                                     const simd32uint8_tpl& b, F&& f) {
    simd32uint8_tpl c;
    for (int j = 0; j < 32; j++) {
  ...
  simd32uint8_tpl operator&(
    const simd256bit_tpl<SIMDLevel::NONE>& other) const {
    return binary_func(*this, simd32uint8_tpl(other),
...
struct simd8uint32_tpl<SIMDLevel::NONE> : simd256bit_tpl<SIMDLevel::NONE> {
  ...
  simd8uint32_tpl unzip() const {
    const uint32_t ret[] = {u32[0], u32[2], u32[4], u32[6],
                            u32[1], u32[3], u32[5], u32[7]};
...
inline void cmplt_min_max_fast(
  ...
  for (size_t i = 0; i < 8; i++) {
    ...
    minIndices.u32[i] = flag ? candidateIndices.u32[i] : currentIndices.u32[i];
    maxValues.u32[i] = !flag ? candidateValues.u32[i] : currentValues.u32[i];
    maxIndices.u32[i] = !flag ? candidateIndices.u32[i] : currentIndices.u32[i];
  }
}
...
struct simd8float32_tpl<SIMDLevel::NONE> : simd256bit_tpl<SIMDLevel::NONE> {
  ...
  template <typename F>
  static simd8float32_tpl binary_func(const simd8float32_tpl& a,
                                      const simd8float32_tpl& b, F&& f) {
    simd8float32_tpl c;
    for (int j = 0; j < 8; j++) {
  ...
  simd8float32_tpl operator*(const simd8float32_tpl& other) const {
    return binary_func(*this, other, [](float a, float b) { return a * b; });
  }
...
inline void cmplt_min_max_fast(
  ...
  for (size_t i = 0; i < 8; i++) {
    ...
    minIndices.u32[i] = flag ? candidateIndices.u32[i] : currentIndices.u32[i];
    maxValues.f32[i] = !flag ? candidateValues.f32[i] : currentValues.f32[i];
    maxIndices.u32[i] = !flag ? candidateIndices.u32[i] : currentIndices.u32[i];
  }
}
