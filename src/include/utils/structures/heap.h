...
#ifndef HYPERVEC_Heap_h
#define HYPERVEC_Heap_h
 ...
 */
template <class C>
inline void heap_push(size_t k, std::pair<typename C::T, typename C::TI>* bh,
                      typename C::T val, typename C::TI id) {
  bh--; /* Use 1-based indexing for easier node->child translation */
  size_t i = k, i_father;
  while (i > 1) {
 ...
 */
template <class C>
inline void heap_replace_top(size_t k,
...
struct HeapArray {
   ...
   */
  void AddnWithIds(size_t nj, const T* vin, const TI* id_in = nullptr,
                     int64_t id_stride = 0, size_t i0 = 0, int64_t ni = -1);

  /** same as AddnWithIds, but for just a subset of queries
   *
   * @param nsubset  number of query entries to update
   * @param subset   indexes of queries to update, in 0..nh-1, size nsubset
   */
  void AddnQuerySubsetWithIds(size_t nsubset, const TI* subset, size_t nj,
   ...
   */
  void PerLineExtrema(T* vals_out, TI* idx_out) const;
};
