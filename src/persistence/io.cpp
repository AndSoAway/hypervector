...
int IOReader::filedescriptor() {
  HYPERVEC_THROW_MSG("IOReader does not support memory mapping");
}

int IOWriter::filedescriptor() {
  HYPERVEC_THROW_MSG("IOWriter does not support memory mapping");
}
...
FileIOReader::FileIOReader(const char* fname) {
  name = fname;
  f = fopen(fname, "rb");
  HYPERVEC_THROW_IF_NOT_FMT(f, "could not open %s for reading: %s", fname,
                            strerror(errno));
  need_close = true;
}
...
FileIOWriter::FileIOWriter(const char* fname) {
  name = fname;
  f = fopen(fname, "wb");
  HYPERVEC_THROW_IF_NOT_FMT(f, "could not open %s for writing: %s", fname,
                            strerror(errno));
  need_close = true;
}
...
size_t BufferedIOWriter::operator()(const void* ptr, size_t unitsize,
                                    size_t nitems) {
  size_t size = unitsize * nitems;
  if (size == 0) {
    return 0;
  ...
  while (size > 0) {
    ...
    do {
      ...
      HYPERVEC_THROW_IF_NOT(written > 0);
      ofs_2 += written;
    } while (ofs_2 != bsz);
