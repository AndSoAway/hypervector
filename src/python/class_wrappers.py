...
def _numeric_to_str(numeric_type):
    if numeric_type == hypervec.kFloat32:
        return 'float32'
    elif numeric_type == hypervec.kFloat16:
        return 'float16'
    elif numeric_type == hypervec.kInt8:
        return 'int8'
    else:
        raise ValueError("numeric type must be either hypervec.kFloat32, hypervec.kFloat16, or hypervec.kInt8")
...
def handle_Index(the_class):
    ...
    def replacement_setattr(self, name, value):
        ...
        # Allow internal hypervec attributes used during construction/operation
    ...
    def replacement_add(self, x, numeric_type=hypervec.kFloat32):
    ...
    def replacement_add_with_ids(self, x, ids, numeric_type = hypervec.kFloat32):
        ...
        if numeric_type == hypervec.kFloat32:
            ...
            self.AddWithIdsEx(n, swig_ptr(x), numeric_type, swig_ptr(ids))
    ...
    def replacement_train(
        self, x, *, numeric_type=hypervec.kFloat32, xq_train=None
    ):
        ...
        if xq_train is not None:
            if numeric_type != hypervec.kFloat32:
                raise TypeError(
                    "xq_train is only supported for numeric_type hypervec.kFloat32"
                )
    ...
    def replacement_search(self, x, k, *, params=None, D=None, I=None, numeric_type = hypervec.kFloat32):
        """Find the k nearest neighbors of the set of vectors x in the index.
    ...
    def replacement_remove_ids(self, x):
        ...
        Parameters
        ----------
        x : array_like or hypervec.IDSelector
    ...
    def replacement_sa_encode(self, x, codes=None):
        ...
        if codes is None:
            codes = np.empty((n, self.SaCodeSize()), dtype=np.uint8)
        else:
            assert codes.shape == (n, self.SaCodeSize())
    ...
    def replacement_permute_entries(self, perm):
        ...
        self.permute_entries_c(hypervec.swig_ptr(perm))

    replace_method(the_class, 'Add', replacement_add)
    replace_method(the_class, 'AddWithIds', replacement_add_with_ids)
    replace_method(the_class, 'Assign', replacement_assign)
    replace_method(the_class, 'Train', replacement_train)
    replace_method(the_class, 'Search', replacement_search)
    replace_method(the_class, 'RemoveIds', replacement_remove_ids)
    replace_method(the_class, 'Reconstruct', replacement_reconstruct)
    replace_method(the_class, 'ReconstructBatch',
                   replacement_reconstruct_batch)
    replace_method(the_class, 'ReconstructN', replacement_reconstruct_n)
    replace_method(the_class, 'RangeSearch', replacement_range_search)
    replace_method(the_class, 'update_vectors', replacement_update_vectors,
                   ignore_missing=True)
    replace_method(the_class, 'SearchAndReconstruct',
    ...
    replace_method(the_class, 'SaEncode', replacement_sa_encode)
    replace_method(the_class, 'SaDecode', replacement_sa_decode)
    replace_method(the_class, 'AddSaCodes', replacement_add_sa_codes)
    replace_method(the_class, 'PermuteEntries', replacement_permute_entries,
                   ignore_missing=True)
    ...
    def index_getstate(self):
        return {"this": hypervec.serialize_index(self).tobytes()}

    def index_setstate(self, st):
        index2 = hypervec.deserialize_index(np.frombuffer(st["this"], dtype="uint8"))
...
def handle_CodePacker(the_class):
    ...
    def replacement_pack_1(self, x, offset, block):
        ...
        self.pack_1_c(swig_ptr(x), offset, hypervec.swig_ptr(block))
    ...
    def replacement_unpack_1(self, block, offset):
        ...
        self.unpack_1_c(hypervec.swig_ptr(block), offset, swig_ptr(x))
        return x
...
def handle_IDSelectorSubset(the_class, class_owns, force_int64=True):
    ...
    def replacement_init(self, *args):
        if len(args) == 1:
            ...
            args = (len(subset), hypervec.swig_ptr(subset))
            if not class_owns:
...
def handle_Tensor2D(the_class):
    ...
    def numpy(self):
        ...
        hypervec.memcpy(hypervec.swig_ptr(shape), self.shape, shape.nbytes)
        return hypervec.vector_to_array(self.v).reshape(shape[0], shape[1])
...
def handle_Linear(the_class):
    ...
    def from_torch(self, linear):
        ...
        if linear.bias is not None:
            ...
            hypervec.copy_array_to_vector(linear.bias.data.numpy(), self.bias)
    ...
    def from_array(self, array, bias=None):
        ...
        if bias is not None:
            ...
            hypervec.copy_array_to_vector(bias, self.bias)
...
def handle_shard_ivf_index_centroids(func):
    def wrapper(*args, **kwargs):
        ...
        if len(args) > 3 and args[3] is not None:
            args[3] = hypervec.PyCallbackShardingFunction(args[3])
