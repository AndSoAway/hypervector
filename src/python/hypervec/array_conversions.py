...
def _make_deprecated_swig_class(deprecated_name, base_name):
    ...
    -------
    None
        However, the deprecated class gets added to the hypervec namespace
    """
    base_class = globals()[base_name]

    def new_meth(cls, *args, **kwargs):
        msg = f"The class hypervec.{deprecated_name} is deprecated in favour of hypervec.{base_name}!"
    ...
    # this ends up adding the class to the "hypervec" namespace, in a way that it
    # is available both through "import hypervec" and "from hypervec import *"
...
# mapping from vector names in swighypervec.swig and the numpy dtype names
