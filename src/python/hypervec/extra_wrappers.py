...
# not linting this file because it imports * from swighypervec, which
...
def kmin(array, k):
    ...
    ha = hypervec.float_maxheap_array_t()
    ha.ids = swig_ptr(I)
    ha.val = swig_ptr(D)
    ha.nh = m
    ha.k = k
...
def kmax(array, k):
    ...
    I = np.zeros((m, k), dtype='int64')
    D = np.zeros((m, k), dtype='float32')
    ha = hypervec.float_minheap_array_t()
    ha.ids = swig_ptr(I)
    ha.val = swig_ptr(D)
    ha.nh = m
    ha.k = k
...
def pairwise_distances(xq, xb, metric=kMetricL2, metric_arg=0):
    ...
    if metric == kMetricL2:
        pairwise_L2sqr(
            ...
            nb, swig_ptr(xb),
            swig_ptr(dis))
    elif metric == kMetricInnerProduct:
        dis[:] = xq @ xb.T
    else:
...
def bucket_sort(tab, nbucket=None, nt=0):
    ...
    bucket_sort_c(
        tab.size, hypervec.swig_ptr(tab.view('uint64')),
        nbucket, hypervec.swig_ptr(lims), hypervec.swig_ptr(perm),
        nt
    )
    return lims, perm
...
def matrix_bucket_sort_inplace(tab, nbucket=None, nt=0):
    ...
    matrix_bucket_sort_inplace_c(
        nrow, ncol, hypervec.swig_ptr(tab),
        nbucket, hypervec.swig_ptr(lims),
        nt
    )
    return lims
...
class MapInt64ToInt64:
    ...
    def __init__(self, capacity):
        ...
        hypervec.hashtable_int64_to_int64_init(self.log2_capacity, swig_ptr(self.tab))

    def Add(self, keys, vals):
        n, = keys.shape
        assert vals.shape == (n,)
        hypervec.hashtable_int64_to_int64_add(
    ...
    def lookup(self, keys):
        ...
        hypervec.hashtable_int64_to_int64_lookup(
            self.log2_capacity, swig_ptr(self.tab),
            n, swig_ptr(keys), swig_ptr(vals))
        return vals
...
def knn(xq, xb, k, metric=kMetricL2, metric_arg=0.0):
    """
    ...
    metric : MetricType, optional
        distance measure to use (either kMetricL2 or kMetricInnerProduct)
...
def knn_hamming(xq, xb, k, variant="hc"):
    ...
    if variant == "hc":
        heap = hypervec.int_maxheap_array_t()
        heap.k = k
        heap.nh = nq
        heap.ids = hypervec.swig_ptr(I)
        heap.val = hypervec.swig_ptr(D)
        hypervec.hammings_knn_hc(
            heap, hypervec.swig_ptr(xq), hypervec.swig_ptr(xb), nb,
            d, 1
        )
    elif variant == "mc":
        hypervec.hammings_knn_mc(
            hypervec.swig_ptr(xq), hypervec.swig_ptr(xb), nq, nb, k, d,
            hypervec.swig_ptr(D), hypervec.swig_ptr(I)
        )
    else:
    ...
    def Train(self, x, weights=None, init_centroids=None):
        ...
        if self.cp.__class__ == ClusteringParameters:
            ...
            if init_centroids is not None:
                ...
                assert d2 == d
                hypervec.copy_array_to_vector(init_centroids.ravel(), clus.centroids)
        ...
        centroids = hypervec.vector_float_to_array(clus.centroids)
