...
class HypervecServerEngine:
    ...
    def flush(self, collection_name: str) -> dict[str, Any]:
        ...
        with self._lock_for(collection_name).write_lock():
            ...
            # flush 后重建标量内存缓存，保证查询走内存而非 SQLite。
            self._scalar_cache[collection_name] = self.scalar_store.load_all_scalars(
                collection_name
            )
            return {
                "flushed": True,
    ...
    def load_collection(self, collection_name: str) -> dict[str, Any]:
        collection_name = self.validate_collection_name(collection_name)
        with self._lock_for(collection_name).write_lock():
            return self._load_collection_unlocked(collection_name)

    def _load_collection_unlocked(self, collection_name: str) -> dict[str, Any]:
        meta = self._meta_or_raise(collection_name)
        index_path = Path(meta.index_path)
        ...
        # 加载索引时一次性把标量字段读入内存，查询期间旁路 SQLite 回表。
        self._scalar_cache[collection_name] = self.scalar_store.load_all_scalars(
            collection_name
        )
        return {
        ...
        *,
        ...
        if collection_name not in self._indexes:
            with lock.write_lock():
                if collection_name not in self._indexes:
                    self._load_collection_unlocked(collection_name)
        ...
        with lock.read_lock():
            ...
            # 自适应候选数：无过滤条件时只取 limit 个，避免不必要的回表开销；
            # 有过滤条件时过取 8 倍，留出过滤后仍能凑够 limit 的余量。
            if filter:
            ...
            for q_labels, q_distances in zip(labels, distances):
                pairs = [
                    ...
                    for label, dist in zip(q_labels, q_distances)
                    if int(label) >= 0
                ]
                row_ids = [rid for rid, _ in pairs]
                if cache is not None:
                    scalars = [cache.get(rid) for rid in row_ids]
                else:
                    scalars = self.scalar_store.get_by_row_ids(collection_name, row_ids)
                hits = []
                for (row_id, dist), scalar in zip(pairs, scalars):
                    if scalar is None:
                        continue
                    row = {
