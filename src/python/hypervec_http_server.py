...
def _require_fastapi():
    ...
    return Body, FastAPI, HTTPException, Query, Request, FileResponse, BaseModel, Field
...
def create_app(
    *,
    ...
    Body, FastAPI, HTTPException, Query, Request, FileResponse, BaseModel, Field = _require_fastapi()
    ...
    examples = {
        "HNSW": {
            ...
            "example_code": {
                "Python": {
                    "create": "index_params.add_index(field_name='vector', index_type='HNSW', metric_type='L2', params={'M': 32, 'ef_construction': 200})",
                    "search": "client.search(collection_name='wiki_hnsw_1m', data=[query], limit=10, search_params={'ef_search': 128})",
                },
                "curl": "curl -X POST http://localhost:8080/collections/wiki_hnsw_1m/search -H 'Content-Type: application/json' -d '{\"data\":[QUERY],\"limit\":10,\"search_params\":{\"ef_search\":128},\"output_fields\":[\"id\"]}'",
            },
            "performance_tips": ["提高 ef_search 可提升召回但增加延迟", "提高 M 可提升图质量但增加内存"],
            "real_world_examples": ["知识库召回", "语义搜索", "推荐候选召回"],
        },
        "Flat": {
            "name": "Flat",
            ...
            "example_code": {"Python": "index_params.add_index(field_name='vector', index_type='Flat', metric_type='L2')"},
            "performance_tips": ["适合做召回率对照基准"],
            "real_world_examples": ["小集合精确检索", "离线评测"],
        },
        "IVF": {
            "name": "IVF",
            ...
            "example_code": {"Python": "index_params.add_index(field_name='vector', index_type='IVF', metric_type='L2', params={'nlist': 1024})"},
            "performance_tips": ["提高 nprobe 可提升召回但增加延迟"],
            "real_world_examples": ["大规模图片检索", "文本向量粗召回"],
        },
        "IVFPQ": {
            "name": "IVFPQ",
            ...
            "example_code": {"Python": "index_params.add_index(field_name='vector', index_type='IVFPQ', metric_type='L2', params={'nlist': 1024, 'm': 16, 'nbits': 8})"},
            "performance_tips": ["增大 nprobe 可提升召回", "调整 m 和 nbits 平衡压缩率与精度"],
            "real_world_examples": ["超大规模低内存检索"],
        },
        "IVFLVQ": {
            "name": "IVFLVQ",
            ...
            "parameters": {"nlist": "聚类中心数", "nlocal": "局部量化参数", "nbits": "编码位数"},
            "example_code": {"Python": "index_params.add_index(field_name='vector', index_type='IVFLVQ', metric_type='L2', params={'nlist': 1024})"},
            "performance_tips": ["根据数据分布调整量化参数"],
            "real_world_examples": ["内存受限向量服务"],
        },
        "PQ": {
            "name": "PQ",
            ...
            "example_code": {"Python": "index_params.add_index(field_name='vector', index_type='PQ', metric_type='L2', params={'m': 16, 'nbits': 8})"},
            "performance_tips": ["增加编码位数通常可提升精度"],
            "real_world_examples": ["低成本海量向量存储"],
        },
        "LVQ": {
            "name": "LVQ",
            ...
            "example_code": {"Python": "index_params.add_index(field_name='vector', index_type='LVQ', metric_type='L2')"},
            "performance_tips": ["先用小样本验证量化参数"],
            "real_world_examples": ["内存带宽敏感检索"],
        },
        "HNSWFlat": {
            "name": "HNSWFlat",
...
def main() -> None:
    ...
    parser.add_argument(
        "--workers",
        type=int,
        default=int(os.environ.get("HYPERVEC_HTTP_WORKERS", "1")),
