...
opt_env_variable_name = "HYPERVEC_OPT_LEVEL"
...
if has_AVX512_SPR:
    try:
        logger.info("Loading hypervec with AVX512-SPR support.")
        from .swighypervec_avx512_spr import *
        logger.info("Successfully loaded hypervec with AVX512-SPR support.")
        loaded = True
...
if has_AVX512 and not loaded:
    try:
        logger.info("Loading hypervec with AVX512 support.")
        from .swighypervec_avx512 import *
        logger.info("Successfully loaded hypervec with AVX512 support.")
        loaded = True
    except ImportError as e:
...
if has_AVX2 and not loaded:
    try:
        logger.info("Loading hypervec with AVX2 support.")
        from .swighypervec_avx2 import *
        logger.info("Successfully loaded hypervec with AVX2 support.")
        loaded = True
    except ImportError as e:
...
if has_SVE and not loaded:
    try:
        ...
        from .swighypervec_sve import *
        logger.info("Successfully loaded hypervec with SVE support.")
        loaded = True
    except ImportError as e:
...
if not loaded:
    try:
        # we import * so that the symbol X can be accessed as hypervec.X
        logger.info("Loading hypervec.")
        from .swighypervec import *
        logger.info("Successfully loaded hypervec.")
    except ModuleNotFoundError:
        formatted_ins_sets = ", ".join(supported_instruction_sets())

        message = (
            f"No module named 'hypervec.swighypervec' found. To fix this, you must "
            f"do both of the following:\n"
            f"A) Set the correct HYPERVEC_OPT_LEVEL value when executing "
            f"'cmake'.\n"
            ...
            f"system, you can set the HYPERVEC_OPT_LEVEL=avx512_spr "
            f"to build the SWIG wrapper with 'AVX512-SPR' support.\n"
            f"You will have to build the 'swighypervec_avx512_spr' "
            f"target in this case.\n"
            f"- If 'AVX512' (case insensitive) is supported on your system, "
            f"you can set the HYPERVEC_OPT_LEVEL=avx512 to build the SWIG wrapper "
            f"with 'AVX512' support.\n"
            f"You will have to build the 'swighypervec_avx512' target in this "
            f"case.\n"
            f"- If 'AVX2' (case sensitive) is supported on your system, you "
            f"can set the HYPERVEC_OPT_LEVEL=AVX2 to build the SWIG wrapper "
            f"with 'AVX2' support.\n"
            f"You will have to build the 'swighypervec_avx2' target in this "
            f"case.\n"
            f"- If 'SVE' (case sensitive) is supported on your system, you can "
            f"set the HYPERVEC_OPT_LEVEL=SVE to build the SWIG wrapper with "
            f"'SVE' support.\n"
            f"You will have to build the 'swighypervec_sve' target in this "
            f"case.\n"
            f"- If none of the above instruction sets are supported on your "
            f"system, you can execute 'cmake' without setting the "
            f"HYPERVEC_OPT_LEVEL variable and build the 'swighypervec' target."
        )
