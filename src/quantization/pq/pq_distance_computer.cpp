...
void PQDistanceComputer::SetQuery(const float* x) {
  const size_t table_size =
    static_cast<size_t>(pq.M) * static_cast<size_t>(pq.ksub);
  if (dis_table.size() != table_size) {
    dis_table.resize(table_size);
  }
  pq.ComputeDistanceTable(x, dis_table.data());
}
...
float PQDistanceComputer::symmetric_dis(idx_t i, idx_t j) {
  const size_t d = static_cast<size_t>(pq.d);
  if (decode_buf_a.size() != d) {
    decode_buf_a.resize(d);
    decode_buf_b.resize(d);
  }
  pq.Decode(codes + static_cast<size_t>(i) * code_size, decode_buf_a.data());
  pq.Decode(codes + static_cast<size_t>(j) * code_size, decode_buf_b.data());
  float acc = 0.0f;
