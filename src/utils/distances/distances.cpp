...
void fvec_inner_product_batch_4(const float* x, const float* y0,
                                const float* y1, const float* y2,
                                const float* y3, const size_t d, float& dis0,
                                float& dis1, float& dis2, float& dis3) {
  fvec_inner_product_batch_4_dispatch(x, y0, y1, y2, y3, d, dis0, dis1, dis2,
                                      dis3);
}

void fvec_L2sqr_batch_4(const float* x, const float* y0, const float* y1,
...
void fvec_L2sqr_ny_transposed(float* dis, const float* x, const float* y,
...
size_t fvec_L2sqr_ny_nearest(float* distances_tmp_buffer, const float* x,
                             const float* y, size_t d, size_t ny) {
  return fvec_L2sqr_ny_nearest_dispatch(distances_tmp_buffer, x, y, d, ny);
}

size_t fvec_L2sqr_ny_nearest_y_transposed(float* distances_tmp_buffer,
                                          const float* x, const float* y,
                                          const float* y_sqlen, size_t d,
                                          size_t d_offset, size_t ny) {
  return fvec_L2sqr_ny_nearest_y_transposed_dispatch(distances_tmp_buffer, x, y,
                                                     y_sqlen, d, d_offset, ny);
}
...
void compute_PQ_dis_tables_dsub2(size_t d, size_t ksub,
                                 const float* all_centroids, size_t nx,
                                 const float* x, bool is_inner_product,
                                 float* dis_tables) {
  compute_PQ_dis_tables_dsub2_dispatch(d, ksub, all_centroids, nx, x,
...
// inside OMP when IndexIVFSpectralHash::SetQuery()
// calls fvec_renorm_L2. SetQuery() is always
...
void exhaustive_inner_product_seq(const float* x, const float* y, size_t d,
                                  size_t nx, size_t ny,
...
template <class BlockResultHandler>
void exhaustive_L2sqr_seq(const float* x, const float* y, size_t d, size_t nx,
                          size_t ny, BlockResultHandler& res) {
  using SingleResultHandler = typename BlockResultHandler::SingleResultHandler;
  [[maybe_unused]] int nt = std::min(int(nx), omp_get_max_threads());
...
/** Find the nearest neighbors for nx queries in a set of ny vectors */
template <class BlockResultHandler>
void exhaustive_inner_product_blas(const float* x, const float* y, size_t d,
                                   size_t nx, size_t ny,
                                   BlockResultHandler& res) {
  ...
  for (size_t i0 = 0; i0 < nx; i0 += bs_x) {
    ...
    for (size_t j0 = 0; j0 < ny; j0 += bs_y) {
      ...
      res.AddResults(j0, j1, ip_block.get());
    }
    res.end_multiple();
...
// the distances
template <class BlockResultHandler>
void exhaustive_L2sqr_blas_default_impl(const float* x, const float* y,
...
template <class BlockResultHandler>
void exhaustive_L2sqr_blas(const float* x, const float* y, size_t d, size_t nx,
                           size_t ny, BlockResultHandler& res,
                           const float* y_norms = nullptr) {
...
void exhaustive_L2sqr_blas_cmax_avx2(
  const float* x, const float* y, size_t d, size_t nx, size_t ny,
  Top1BlockResultHandler<CMax<float, int64_t>>& res, const float* y_norms) {
  // BLAS does not like empty matrices
  if (nx == 0 || ny == 0) {
    return;
  }
    ...
    for (size_t j0 = 0; j0 < ny; j0 += bs_y) {
      ...
      for (int64_t i = i0; i < i1; i++) {
        ...
        for (; idx_j < (count / 16) * 16; idx_j += 16, ip_line += 16) {
          ...
          const __m256 comparison_0 =
            _mm256_cmp_ps(min_distances, distances_0, _CMP_LE_OS);

          // update min distances and indices with closest vectors if
          // needed.
          min_distances =
            _mm256_blendv_ps(distances_0, min_distances, comparison_0);
          min_indices = _mm256_castps_si256(
            _mm256_blendv_ps(_mm256_castsi256_ps(current_indices),
                             _mm256_castsi256_ps(min_indices), comparison_0));
          current_indices = _mm256_add_epi32(current_indices, indices_delta);

          // compare the new distances to the min distances
          // for each of the second group of 8 AVX2 components.
          const __m256 comparison_1 =
            _mm256_cmp_ps(min_distances, distances_1, _CMP_LE_OS);

          // update min distances and indices with closest vectors if
          // needed.
          min_distances =
            _mm256_blendv_ps(distances_1, min_distances, comparison_1);
          min_indices = _mm256_castps_si256(
            _mm256_blendv_ps(_mm256_castsi256_ps(current_indices),
                             _mm256_castsi256_ps(min_indices), comparison_1));
          current_indices = _mm256_add_epi32(current_indices, indices_delta);
        }
        ...
        for (size_t jv = 0; jv < 8; jv++) {
          ...
          } else if (current_min_distance == distance_candidate &&
                     current_min_index > index_candidate) {
            current_min_index = index_candidate;
          }
        }
        ...
        //
        res.AddResult(i, current_min_distance, current_min_index);
      }
    }
  ...
  for (size_t i0 = 0; i0 < nx; i0 += bs_x) {
    ...
    for (size_t j0 = 0; j0 < ny; j0 += bs_y) {
      ...
      for (int64_t i = i0; i < i1; i++) {
        ...
        svprfw(svwhilelt_b32_u64(lanes, count), ip_line + lanes, SV_PLDL1KEEP);
        ...
        for (size_t idx_j = 0; idx_j < count;
          ...
          svprfw(svwhilelt_b32_u64(idx_j + lanes * 2, count),
                 ip_line + lanes * 2, SV_PLDL1KEEP);
          svprfw(svwhilelt_b32_u64(idx_j + lanes * 3, count),
                 ip_line + lanes * 3, SV_PLDL1KEEP);
          ...
          // load values for norms
          const auto y_norm_0 = svld1_f32(mask_0, y_norms + idx_j + j0 + 0);
          const auto y_norm_1 = svld1_f32(mask_1, y_norms + idx_j + j0 + lanes);
          ...
          // x_norm[i] was dropped off because it is a constant for a
          // given i. We'll deal with it later.
          const auto distances_0 = svmla_n_f32_z(mask_0, y_norm_0, ip_0, -2.f);
          const auto distances_1 = svmla_n_f32_z(mask_1, y_norm_1, ip_1, -2.f);

          // compare the new distances to the min distances
          // for each of the first group of 4 ARM SIMD components.
          auto comparison = svcmpgt_f32(mask_0, min_distances, distances_0);

          // update min distances and indices with closest vectors if
          // needed.
          min_distances = svsel_f32(comparison, distances_0, min_distances);
          min_indices = svsel_u32(comparison, current_indices, min_indices);
          current_indices = svadd_n_u32_x(mask_0, current_indices,
                                          static_cast<uint32_t>(lanes));

          // compare the new distances to the min distances
          // for each of the second group of 4 ARM SIMD components.
          comparison = svcmpgt_f32(mask_1, min_distances, distances_1);

          // update min distances and indices with closest vectors if
          // needed.
          min_distances = svsel_f32(comparison, distances_1, min_distances);
          min_indices = svsel_u32(comparison, current_indices, min_indices);
          current_indices = svadd_n_u32_x(mask_1, current_indices,
                                          static_cast<uint32_t>(lanes));
        }
        ...
        min_distances =
          svadd_n_f32_z(svcmpge_n_f32(mask, min_distances, -x_norms[i]),
                        min_distances, x_norms[i]);
        min_indices =
          svadd_n_u32_x(mask, min_indices, static_cast<uint32_t>(j0));
        mask = svcmple_n_f32(mask, min_distances, res.dis_tab[i]);
        if (svcntp_b32(svptrue_b32(), mask) == 0)
          res.AddResult(i, res.dis_tab[i], res.ids_tab[i]);
        else {
          const auto min_distance = svminv_f32(mask, min_distances);
          const auto min_index = svminv_u32(
            svcmpeq_n_f32(mask, min_distances, min_distance), min_indices);
          res.AddResult(i, min_distance, min_index);
        }
      }
    }
    // Does nothing for SingleBestResultHandler, but
...
void exhaustive_L2sqr_blas<Top1BlockResultHandler<CMax<float, int64_t>>>(
  ...
  exhaustive_L2sqr_blas_default_impl<
    Top1BlockResultHandler<CMax<float, int64_t>>>(x, y, d, nx, ny, res,
                                                  y_norms);
#else
  // run the default implementation
  exhaustive_L2sqr_blas_default_impl<
...
struct Run_search_inner_product {
  using T = void;
  template <class BlockResultHandler>
  void f(BlockResultHandler& res, const float* x, const float* y, size_t d,
         size_t nx, size_t ny) {
...
struct Run_search_L2sqr {
  ...
  template <class BlockResultHandler>
  void f(BlockResultHandler& res, const float* x, const float* y, size_t d,
         size_t nx, size_t ny, const float* y_norm2) {
    if (res.sel || nx < distance_compute_blas_threshold) {
...
void knn_inner_product(const float* x, const float* y, size_t d, size_t nx,
                       size_t ny, size_t k, float* vals, int64_t* ids,
  ...
  if (auto sela = dynamic_cast<const IDSelectorArray*>(sel)) {
    knn_inner_products_by_idx(x, y, sela->ids, d, nx, ny, sela->n, k, vals, ids,
                              0);
    return;
  }

  Run_search_inner_product r;
  dispatch_knn_ResultHandler(nx, vals, ids, k, kMetricInnerProduct, sel, r, x,
                             y, d, nx, ny);
...
void knn_inner_product(const float* x, const float* y, size_t d, size_t nx,
                       ...
                       const IDSelector* sel) {
  HYPERVEC_THROW_IF_NOT(nx == res->nh);
...
void knn_L2sqr(const float* x, const float* y, size_t d, size_t nx, size_t ny,
               size_t k, float* vals, int64_t* ids, const float* y_norm2,
               const IDSelector* sel) {
  int64_t imin = 0;
  if (auto selr = dynamic_cast<const IDSelectorRange*>(sel)) {
    imin = std::max(selr->imin, int64_t(0));
  ...
  Run_search_L2sqr r;
  dispatch_knn_ResultHandler(nx, vals, ids, k, kMetricL2, sel, r, x, y, d, nx,
                             ny, y_norm2);
...
void knn_L2sqr(const float* x, const float* y, size_t d, size_t nx, size_t ny,
               float_maxheap_array_t* res, const float* y_norm2,
               const IDSelector* sel) {
  HYPERVEC_THROW_IF_NOT(res->nh == nx);
...
void range_search_L2sqr(const float* x, const float* y, size_t d, size_t nx,
                        ...
                        const IDSelector* sel) {
  Run_search_L2sqr r;
  dispatch_range_ResultHandler(res, radius, kMetricL2, sel, r, x, y, d, nx, ny,
                               nullptr);
}

void range_search_inner_product(const float* x, const float* y, size_t d,
                                size_t nx, size_t ny, float radius,
                                RangeSearchResult* res, const IDSelector* sel) {
  Run_search_inner_product r;
  dispatch_range_ResultHandler(res, radius, kMetricInnerProduct, sel, r, x, y,
                               d, nx, ny);
}
...
void pairwise_indexed_L2sqr(size_t d, size_t n, const float* x,
                            const int64_t* ix, const float* y,
                            const int64_t* iy, float* dis) {
...
void pairwise_indexed_inner_product(size_t d, size_t n, const float* x,
                                    const int64_t* ix, const float* y,
                                    const int64_t* iy, float* dis) {
#pragma omp parallel for if (n > 1)
  for (int64_t j = 0; j < n; j++) {
    if (ix[j] >= 0 && iy[j] >= 0) {
      dis[j] = fvec_inner_product_dispatch(x + d * ix[j], y + d * iy[j], d);
    } else {
      dis[j] = -INFINITY;
    }
  }
...
void knn_inner_products_by_idx(const float* x, const float* y,
                               ...
                               size_t ny, size_t nsubset, size_t k,
                               float* res_vals, int64_t* res_ids,
                               int64_t ld_ids) {
  if (ld_ids < 0) {
    ld_ids = ny;
  }
...
void knn_L2sqr_by_idx(const float* x, const float* y,
                      const int64_t* __restrict ids, size_t d, size_t nx,
                      size_t ny, size_t nsubset, size_t k, float* res_vals,
                      int64_t* res_ids, int64_t ld_ids) {
  if (ld_ids < 0) {
    ld_ids = ny;
  }
#pragma omp parallel for if (nx > 100)
...
void pairwise_L2sqr(int64_t d, int64_t nq, const float* xq, int64_t nb,
                    const float* xb, float* dis, int64_t ldq, int64_t ldb,
                    int64_t ldd) {
  if (nq == 0 || nb == 0) {
    return;
  }
