...
struct ExtraDistanceComputer : FlatCodesDistanceComputer {
  ...
  ExtraDistanceComputer(const VD& vd, const float* xb, size_t nb,
...
void pairwise_extra_distances(int64_t d, int64_t nq, const float* xq,
                              ...
                              float metric_arg, float* dis, int64_t ldq,
                              int64_t ldb, int64_t ldd) {
  if (nq == 0 || nb == 0) {
    return;
  }
  if (ldq == -1) {
...
void knn_extra_metrics(const float* x, const float* y, size_t d, size_t nx,
  ...
  with_VectorDistance(d, mt, metric_arg, [&](auto vd) {
    ...
    size_t check_period = InterruptCallback::GetPeriodHint(ny * d);
