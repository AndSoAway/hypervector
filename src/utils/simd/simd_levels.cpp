...
SIMDConfig::SIMDConfig(const char** hypervec_simd_level_env) {
  // Support dependency injection for testing
  const char* env_var = hypervec_simd_level_env ? *hypervec_simd_level_env
                                                : getenv("HYPERVEC_SIMD_LEVEL");
...
void SIMDConfig::set_level(SIMDLevel l) {
  if (!is_simd_level_available(l)) {
    HYPERVEC_THROW_FMT("SIMDConfig::set_level: level %s is not available",
                       to_string(l).c_str());
  }
  level = l;
}
  ...
  if (avx_supported) {
    ...
    if (has_avx512f) {
      ...
      if (has_avx512bw && has_avx512cd && has_avx512vl && has_avx512dq) {
        ...
        supported_simd_levels |= (1 << static_cast<int>(SIMDLevel::AVX512));
...
void SIMDConfig::set_level(SIMDLevel l) {
  if (!is_simd_level_available(l)) {
    HYPERVEC_THROW_FMT(
      ...
      to_string(l).c_str(), to_string(level).c_str());
  }
