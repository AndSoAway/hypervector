...
template <typename C>
void HeapArray<C>::AddnWithIds(size_t nj, const T* vin, const TI* id_in,
...
template <typename C>
void HeapArray<C>::AddnQuerySubsetWithIds(size_t nsubset, const TI* subset,
                                              size_t nj, const T* vin,
                                              const TI* id_in,
                                              int64_t id_stride) {
  HYPERVEC_THROW_IF_NOT_MSG(id_in, "anonymous ids not supported");
  if (id_stride < 0) {
    id_stride = nj;
  }
...
template <typename C>
void HeapArray<C>::PerLineExtrema(T* out_val, TI* out_ids) const {
    ...
    for (long i = 0; i < n; i++) {
      ...
      for (j = 0; j < k && heap_size > 0; j++) {
        ...
        if (p < k && I_in[stride * s + p] >= 0) {
          heap_push<C>(++heap_size, heap_vals, shard_ids, D_in[stride * s + p],
                       s);
        }
      }
      for (; j < k; j++) {
...
#define INSTANTIATE(C, distance_t)                                       \
  template void merge_knn_results<int64_t, C<distance_t, int>>(          \
    size_t, size_t, int, const distance_t*, const int64_t*, distance_t*, \
    int64_t*);
...
template <class C>
void reorder_2_heaps(int64_t n, int64_t k, typename C::TI* __restrict labels,
                     float* __restrict distances, int64_t k_base,
...
template void reorder_2_heaps<CMax<float, int64_t>>(
  int64_t n, int64_t k, int64_t* __restrict labels, float* __restrict distances,
  int64_t k_base, const int64_t* __restrict base_labels,
  const float* __restrict base_distances);
