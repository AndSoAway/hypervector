...
void FloatRandn(float* x, size_t n, int64_t seed) {
  // only try to parallelize on large enough arrays
  const size_t nblock = n < 1024 ? 1 : 1024;
...
void RandSmoothVectors(size_t n, size_t d, float* x, int64_t seed) {
  size_t d1 = 10;
  std::vector<float> x1(n * d1);
  FloatRandn(x1.data(), x1.size(), seed);
  std::vector<float> rot(d1 * d);
  FloatRand(rot.data(), rot.size(), seed + 1);
  ...
  FloatRand(scales.data(), d, seed + 2);
