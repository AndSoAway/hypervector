...
void bucket_sort_ref(size_t nval, const uint64_t* vals, uint64_t vmax,
                     int64_t* lims, int64_t* perm) {
  double t0 = getmillisecs();
  memset(lims, 0, sizeof(*lims) * (vmax + 1));
  for (size_t i = 0; i < nval; i++) {
    HYPERVEC_THROW_IF_NOT(vals[i] < vmax);
    lims[vals[i] + 1]++;
  }
  ...
  HYPERVEC_THROW_IF_NOT(lims[vmax] == nval);
...
void bucket_sort_parallel(size_t nval, const uint64_t* vals, uint64_t vmax,
                          int64_t* lims, int64_t* perm, int nt_in) {
  memset(lims, 0, sizeof(*lims) * (vmax + 1));
...
template <class TI>
void bucket_sort_inplace_ref(size_t nrow, size_t ncol, TI* vals, TI nbucket,
                             int64_t* lims) {
  double t0 = getmillisecs();
  size_t nval = nrow * ncol;
  HYPERVEC_THROW_IF_NOT(nbucket <
                        nval);  // unclear what would happen in this case...

  memset(lims, 0, sizeof(*lims) * (nbucket + 1));
  for (size_t i = 0; i < nval; i++) {
    HYPERVEC_THROW_IF_NOT(vals[i] < nbucket);
    lims[vals[i] + 1]++;
  }
  double t1 = getmillisecs();
  // compute cumulative sum
  ...
  HYPERVEC_THROW_IF_NOT(lims[nbucket] == nval);
  double t2 = getmillisecs();
...
struct ToWrite {
  ...
  void bucket_sort() {
    HYPERVEC_THROW_IF_NOT(buckets.size() == rows.size());
    lims.resize(nbucket + 1);
    ...
    HYPERVEC_THROW_IF_NOT(lims[nbucket] == buckets.size());
...
template <class TI>
void bucket_sort_inplace_parallel(size_t nrow, size_t ncol, TI* vals,
                                  TI nbucket, int64_t* lims, int nt_in) {
  ...
  size_t init_to_write =
    std::max(size_t(nbucket),
        ...
        if (verbose > 2) {
          for (size_t b = 0; b < nbucket; b++) {
            ...
            for (size_t i = lims[b]; i < lims[b + 1]; i++) {
              printf(" %s%d", ptrs[b] == i ? ">" : "", int(vals[i]));
            }
            printf(" %s] %s\n", ptrs[b] == lims[b + 1] ? ">" : "",
                   did_wrap[b] ? "w" : "");
          }
...
void bucket_sort(size_t nval, const uint64_t* vals, uint64_t vmax,
                 int64_t* lims, int64_t* perm, int nt) {
  if (nt == 0) {
...
void matrix_bucket_sort_inplace(size_t nrow, size_t ncol, int32_t* vals,
                                int32_t vmax, int64_t* lims, int nt) {
  if (nt == 0) {
...
void matrix_bucket_sort_inplace(size_t nrow, size_t ncol, int64_t* vals,
                                int64_t vmax, int64_t* lims, int nt) {
  if (nt == 0) {
    bucket_sort_inplace_ref(nrow, ncol, vals, vmax, lims);
  } else {
    bucket_sort_inplace_parallel(nrow, ncol, vals, vmax, lims, nt);
...
int log2_capacity_to_log2_nbucket(int log2_capacity) {
  return log2_capacity < 12 ? 0 : log2_capacity < 20 ? log2_capacity - 12 : 10;
}
...
void hashtable_int64_to_int64_add(int log2_capacity, int64_t* tab, size_t n,
...
#pragma omp parallel for
  ...
  bucket_sort(n, bucket_no.data(), nbucket, lims.data(), perm.data(),
              omp_get_max_threads());
...
#pragma omp parallel for reduction(+ : num_errors)
  ...
  HYPERVEC_THROW_IF_NOT_MSG(num_errors == 0, "hashtable capacity exhausted");
}

void hashtable_int64_to_int64_lookup(int log2_capacity, const int64_t* tab,
                                     size_t n, const int64_t* keys,
                                     int64_t* vals) {
  size_t capacity = (size_t)1 << log2_capacity;
