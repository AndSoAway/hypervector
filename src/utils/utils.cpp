...
// placeholder for additional compile options (previously used for GPU)
...
size_t get_mem_usage_kb() {
  ...
  HYPERVEC_THROW_IF_NOT_MSG(f, "cannot open proc status file");
  size_t sz = 0;
  for (;;) {
    char buf[256];
    if (!fgets(buf, 256, f))
...
void bincode_hist(size_t n, size_t nbits, const uint8_t* codes, int* hist) {
  HYPERVEC_THROW_IF_NOT(nbits % 8 == 0);
  size_t d = nbits / 8;
...
const float* fvecs_maybe_subsample(size_t d, size_t* n, size_t nmax,
                                   const float* x, bool verbose, int64_t seed) {
  if (*n <= nmax)
  ...
  if (verbose) {
    printf(
      "  Input training set too big (max size is %zd), sampling "
      "%zd / %zd vectors\n",
      nmax, n2, *n);
  }
...
void CodeSet::insert(size_t n, const uint8_t* codes, bool* inserted) {
  for (size_t i = 0; i < n; i++) {
    auto res = s.insert(std::vector<uint8_t>(codes + i * d, codes + i * d + d));
