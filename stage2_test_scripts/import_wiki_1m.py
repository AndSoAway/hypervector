...
def import_wiki_1m(
    fbin_path='/root/vector/data/wiki_all_1M/base.1M.fbin',
    ...
    print("-" * 80)
    try:
        client = HypervecClient(host, timeout=timeout)
        # 测试连接
    ...
    try:
        ...
        if collection_name in existing:
            ...
            if not drop_existing:
                print("❌ 取消导入：如需删除并重建，请显式传入 --drop-existing")
                return False
...
def main():
    ...
    parser.add_argument(
        '--drop-existing',
        action='store_true',
        help='如果 collection 已存在，则删除并重新创建'
    )
    parser.add_argument(
        '--timeout',
        type=float,
        default=2400.0,
        help='HTTP 请求超时时间，单位秒，默认 2400 秒'
    )
