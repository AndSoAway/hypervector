 ...
 * source tree.
 *
 * IndexHNSWLVQ demo: train, add, sweep ef_search, and measure recall against
 * brute force.
 */
...
int main() {
  ...
  std::cout << "\n------------------------------------------------" << std::endl;
  std::cout << "ef_search   Recall@" << k << "    Search ms   ms/query"
            << std::endl;
  std::cout << "------------------------------------------------" << std::endl;
  ...
  for (int ef : {64, 128, 256, 512, 1024}) {
    ...
    printf("  %4d        %5.1f%%    %8.2f    %8.3f\n", ef,
           recall * 100.0f, search_ms, search_ms / nq);
  }
  std::cout << "------------------------------------------------" << std::endl;
