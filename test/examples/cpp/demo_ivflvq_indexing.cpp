 ...
 * source tree.
 *
 * IndexIVFLVQ demo — train, add, sweep nprobe, measure recall vs brute-force.
 */
...
int main() {
  std::cout << "IVF+LVQ Index Demo" << std::endl;
  std::cout << "==================" << std::endl;
  ...
  std::cout << "\n========================================" << std::endl;
  std::cout << "nprobe   Recall@10    Search ms" << std::endl;
  std::cout << "----------------------------------------" << std::endl;
  ...
  for (int nprobe : {1, 2, 4, 8, 16, 32, 64}) {
    ...
    float recall = (float)correct / (float)(nq * k);
