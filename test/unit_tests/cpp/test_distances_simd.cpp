...
static void fvec_inner_products_ny_ref(
    ...
    for (size_t i = 0; i < ny; i++) {
        ip[i] = hypervec::fvec_inner_product(x, y, d);
        y += d;
    }
}
...
static void fvec_L2sqr_ny_ref(
        ...
        size_t ny) {
    for (size_t i = 0; i < ny; i++) {
        dis[i] = hypervec::fvec_L2sqr(x, y, d);
        y += d;
    }
}
...
TEST(TestFvecL2sqr, distances_L2_squared_y_transposed) {
    ...
    for (const auto d : {1, 2, 3, 4, 5, 6, 7, 8, 16}) {
        ...
        hypervec::fvec_L2sqr_ny_transposed(
                distances.data(),
                x.data(),
                y.data(),
                y_sqlens.data(),
...
TEST(TestFvecL2sqr, nearest_L2_squared_y_transposed) {
    ...
    for (const auto d : {1, 2, 3, 4, 5, 6, 7, 8, 16}) {
        ...
        size_t nearest_idx = hypervec::fvec_L2sqr_ny_nearest_y_transposed(
                buffer.data(),
                x.data(),
                y.data(),
                y_sqlens.data(),
...
TEST(TestFvecL1, manhattan_distance) {
    ...
    for (const auto nrows : {8, 9, 10, 11, 12, 13, 14, 15, 16}) {
        ...
        auto distance = hypervec::fvec_L1(x.data(), y.data(), x.size());
...
TEST(TestFvecLinf, chebyshev_distance) {
    ...
    for (const auto nrows : {8, 9, 10, 11, 12, 13, 14, 15, 16}) {
        ...
        auto distance = hypervec::fvec_Linf(x.data(), y.data(), x.size());
...
TEST(TestFvecMadd, multiple_add) {
    ...
    for (const auto nrows : {8, 9, 10, 11, 12, 13, 14, 15, 16}) {
        ...
        hypervec::fvec_madd(a.size(), a.data(), bf, b.data(), distances.data());
...
TEST(TestFvecAdd, add_array) {
    ...
    for (const auto nrows : {1, 2, 5, 10, 15, 20, 25}) {
        ...
        std::vector<float> distances(nrows);
        hypervec::fvec_add(a.size(), a.data(), b.data(), distances.data());
...
TEST(TestFvecAdd, add_value) {
    ...
    for (const auto nrows : {1, 2, 5, 10, 15, 20, 25}) {
        ...
        std::vector<float> distances(nrows);
        hypervec::fvec_add(a.size(), a.data(), b, distances.data());
