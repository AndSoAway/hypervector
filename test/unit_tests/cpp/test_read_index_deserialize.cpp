...
#include <hypervec/IndexBinary.h>
#include <hypervec/VectorTransform.h>
#include <hypervec/impl/HypervecException.h>
#include <hypervec/impl/io.h>
#include <hypervec/index_io.h>
#include <hypervec/invlists/InvertedLists.h>
#include <hypervec/utils/hamming.h>
...
/// Try to read a float index from the given buffer and expect a HypervecException.
static void expect_read_throws(const std::vector<uint8_t>& data) {
    VectorIOReader reader;
    reader.data = data;
    EXPECT_THROW(read_index_up(&reader), HypervecException);
}

/// Try to read a float index and expect a HypervecException whose message
...
static void expect_read_throws_with(
    ...
    try {
        ...
        FAIL() << "expected HypervecException";
    } catch (const HypervecException& e) {
        EXPECT_NE(
...
TEST(ReadIndexDeserialize, READVECTORByteLimit) {
    ...
    set_deserialization_vector_byte_limit(n_elements * sizeof(float));
    {
        VectorIOReader reader;
        reader.data = buf;
        EXPECT_THROW(read_VectorTransform_up(&reader), HypervecException);
    }
...
static void expect_binary_read_throws(const std::vector<uint8_t>& data) {
    VectorIOReader reader;
    reader.data = data;
    EXPECT_THROW(read_index_binary_up(&reader), HypervecException);
}

/// Try to read a binary index and expect a HypervecException whose message
/// contains the given substring.
...
/// HypervecException whose message contains the given substring.
