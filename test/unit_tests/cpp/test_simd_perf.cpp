...
class SIMDPerfTest : public ::testing::Test {
   protected:
    void SetUp() override {
        original_level = hypervec::SIMDConfig::get_level();
        ...
        hypervec::FloatRand(x.data(), d, 42);
        hypervec::FloatRand(y.data(), ny * d, 43);
    }
    ...
    void verify_dispatch(hypervec::SIMDLevel level) {
        hypervec::SIMDConfig::set_level(level);
        hypervec::SIMDLevel dispatched = hypervec::SIMDConfig::get_dispatched_level();
        ASSERT_EQ(dispatched, level)
                << "Dispatch broken: set_level(" << hypervec::to_string(level)
    ...
    double benchmark_fvec_inner_products_ny(
            hypervec::SIMDLevel level,
            int n_runs = 5000) {
        hypervec::SIMDConfig::set_level(level);

        // Warmup to stabilize CPU frequency
        for (int i = 0; i < 200; i++) {
            hypervec::fvec_inner_products_ny(
    ...
    hypervec::SIMDLevel original_level = hypervec::SIMDLevel::NONE;
    ...
    std::pair<double, double> benchmark_interleaved_best(
            ...
            hypervec::SIMDLevel level_a,
            hypervec::SIMDLevel level_b) {
...
TEST_F(SIMDPerfTest, AVX2FasterThanNONE) {
    if (!hypervec::SIMDConfig::is_simd_level_available(hypervec::SIMDLevel::AVX2)) {
    ...
    verify_dispatch(hypervec::SIMDLevel::NONE);
    verify_dispatch(hypervec::SIMDLevel::AVX2);
    ...
    auto bench_fn = [this](hypervec::SIMDLevel level) {
        return benchmark_fvec_inner_products_ny(level);
    };
    auto [none_time, avx2_time] = benchmark_interleaved_best(
            bench_fn, hypervec::SIMDLevel::NONE, hypervec::SIMDLevel::AVX2);
...
TEST_F(SIMDPerfTest, AVX512FasterThanAVX2IfAvailable) {
    if (!hypervec::SIMDConfig::is_simd_level_available(hypervec::SIMDLevel::AVX512)) {
        GTEST_SKIP() << "AVX512 not available on this machine";
    }

    verify_dispatch(hypervec::SIMDLevel::AVX2);
    verify_dispatch(hypervec::SIMDLevel::AVX512);

    auto bench_fn = [this](hypervec::SIMDLevel level) {
        return benchmark_fvec_inner_products_ny(level);
    };
    auto [avx2_time, avx512_time] = benchmark_interleaved_best(
            bench_fn, hypervec::SIMDLevel::AVX2, hypervec::SIMDLevel::AVX512);
...
TEST_F(SIMDPerfTest, L2sqrAutoVecDispatchWorks) {
    ...
    hypervec::FloatRand(lx.data(), large_d, 44);
    hypervec::FloatRand(ly.data(), large_ny * large_d, 45);

    auto bench_fn = [&](hypervec::SIMDLevel level) {
        hypervec::SIMDConfig::set_level(level);
        // Warmup
        for (int i = 0; i < 100; i++) {
            hypervec::fvec_inner_products_ny(
