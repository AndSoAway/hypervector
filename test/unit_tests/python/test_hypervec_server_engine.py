...
def test_hypervec_server_engine_supported_index_examples_follow_exports():
    ...
    assert "PQ" not in {item["name"] for item in examples}
    assert "LVQ" not in {item["name"] for item in examples}
    assert engine.get_index_example("IVF")["index_type"] == "IndexIVFFlat"
    assert engine.get_index_example("HNSW")["index_type"] == "IndexHNSWFlat"
    assert engine.get_index_example("HNSWFlat")["aliases"] == [
        "HNSW",
        "AutoIndex",
        "IndexHNSWFlat",
    ]
    assert engine.get_index_example("unknown") is None
