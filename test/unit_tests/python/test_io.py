# Copyright (c) 2024 HyperVec Authors. All rights reserved.
#
# This source code is licensed under the Mulan Permissive Software License v2 (the "License") found in the
# LICENSE file in the root directory of this source tree.

import numpy as np
import unittest
import hypervec
import tempfile
import os
import io
import sys
import pickle
import platform
from multiprocessing.pool import ThreadPool
from common_Hypervec_tests import get_dataset_2


d = 32
nt = 2000
nb = 1000
nq = 200


class TestIOVariants(unittest.TestCase):

    def test_io_error(self):
        d, n = 32, 1000
        x = np.random.uniform(size=(n, d)).astype('float32')
        index = hypervec.IndexFlatL2(d)
        index.Add(x)
        fd, fname = tempfile.mkstemp()
        os.close(fd)
        try:
            hypervec.write_index(index, fname)

            # should be fine
            hypervec.read_index(fname)

            with open(fname, 'rb') as f:
                data = f.read()
            # now damage file
            with open(fname, 'wb') as f:
                f.write(data[:int(len(data) / 2)])

            # should make a nice readable exception that mentions the filename
            try:
                hypervec.read_index(fname)
            except RuntimeError as e:
                if fname not in str(e):
                    raise
            else:
                raise

        finally:
            if os.path.exists(fname):
                os.unlink(fname)


class TestCallbacks(unittest.TestCase):

    def do_write_callback(self, bsz):
        d, n = 32, 1000
        x = np.random.uniform(size=(n, d)).astype('float32')
        index = hypervec.IndexFlatL2(d)
        index.Add(x)

        f = io.BytesIO()
        # test with small block size
        writer = hypervec.PyCallbackIOWriter(f.write, 1234)

        if bsz > 0:
            writer = hypervec.BufferedIOWriter(writer, bsz)

        hypervec.write_index(index, writer)
        del writer   # make sure all writes committed

        if sys.version_info[0] < 3:
            buf = f.getvalue()
        else:
            buf = f.getbuffer()

        index2 = hypervec.deserialize_index(np.frombuffer(buf, dtype='uint8'))

        self.assertEqual(index.d, index2.d)
        np.testing.assert_array_equal(
            hypervec.vector_to_array(index.codes),
            hypervec.vector_to_array(index2.codes)
        )

        # This is not a callable function: should raise an exception
        writer = hypervec.PyCallbackIOWriter("blabla")
        self.assertRaises(
            Exception,
            hypervec.write_index, index, writer
        )

    def test_buf_read(self):
        x = np.random.uniform(size=20)

        fd, fname = tempfile.mkstemp()
        os.close(fd)
        try:
            x.tofile(fname)

            with open(fname, 'rb') as f:
                reader = hypervec.PyCallbackIOReader(f.read, 1234)

                bsz = 123
                reader = hypervec.BufferedIOReader(reader, bsz)

                y = np.zeros_like(x)
                reader(hypervec.swig_ptr(y), y.nbytes, 1)

            np.testing.assert_array_equal(x, y)
        finally:
            if os.path.exists(fname):
                os.unlink(fname)

    def do_read_callback(self, bsz):
        d, n = 32, 1000
        x = np.random.uniform(size=(n, d)).astype('float32')
        index = hypervec.IndexFlatL2(d)
        index.Add(x)

        fd, fname = tempfile.mkstemp()
        os.close(fd)
        try:
            hypervec.write_index(index, fname)

            with open(fname, 'rb') as f:
                reader = hypervec.PyCallbackIOReader(f.read, 1234)

                if bsz > 0:
                    reader = hypervec.BufferedIOReader(reader, bsz)

                index2 = hypervec.read_index(reader)

            self.assertEqual(index.d, index2.d)
            np.testing.assert_array_equal(
                hypervec.vector_to_array(index.codes),
                hypervec.vector_to_array(index2.codes)
            )

            # This is not a callable function: should raise an exception
            reader = hypervec.PyCallbackIOReader("blabla")
            self.assertRaises(
                Exception,
                hypervec.read_index, reader
            )
        finally:
            if os.path.exists(fname):
                os.unlink(fname)

    def test_write_callback(self):
        self.do_write_callback(0)

    def test_write_buffer(self):
        self.do_write_callback(123)
        self.do_write_callback(2345)

    def test_read_callback(self):
        self.do_read_callback(0)

    def test_read_callback_buffered(self):
        self.do_read_callback(123)
        self.do_read_callback(12345)

    def test_read_buffer(self):
        d, n = 32, 1000
        x = np.random.uniform(size=(n, d)).astype('float32')
        index = hypervec.IndexFlatL2(d)
        index.Add(x)

        fd, fname = tempfile.mkstemp()
        os.close(fd)
        try:
            hypervec.write_index(index, fname)

            reader = hypervec.BufferedIOReader(
                hypervec.FileIOReader(fname), 1234)

            index2 = hypervec.read_index(reader)

            self.assertEqual(index.d, index2.d)
            np.testing.assert_array_equal(
                hypervec.vector_to_array(index.codes),
                hypervec.vector_to_array(index2.codes)
            )

        finally:
            del reader
            if os.path.exists(fname):
                os.unlink(fname)

    def test_transfer_pipe(self):
        """ transfer an index through a Unix pipe """

        d, n = 32, 1000
        x = np.random.uniform(size=(n, d)).astype('float32')
        index = hypervec.IndexFlatL2(d)
        index.Add(x)
        Dref, Iref = index.Search(x, 10)

        rf, wf = os.pipe()

        # start thread that will decompress the index

        def index_from_pipe():
            reader = hypervec.PyCallbackIOReader(lambda size: os.read(rf, size))
            return hypervec.read_index(reader)

        with ThreadPool(1) as pool:
            fut = pool.apply_async(index_from_pipe, ())

            # write to pipe
            writer = hypervec.PyCallbackIOWriter(lambda b: os.write(wf, b))
            hypervec.write_index(index, writer)

            index2 = fut.get()

            # closing is not really useful but it does not hurt
            os.close(wf)
            os.close(rf)

        Dnew, Inew = index2.Search(x, 10)

        np.testing.assert_array_equal(Iref, Inew)
        np.testing.assert_array_equal(Dref, Dnew)


class PyOndiskInvertedLists:
    """ wraps an OnDisk object for use from C++ """

    def __init__(self, oil):
        self.oil = oil

    def list_size(self, list_no):
        return self.oil.list_size(list_no)

    def get_codes(self, list_no):
        oil = self.oil
        assert 0 <= list_no < oil.lists.size()
        l = oil.lists.at(list_no)
        with open(oil.filename, 'rb') as f:
            f.seek(l.offset)
            return f.read(l.size * oil.code_size)

    def get_ids(self, list_no):
        oil = self.oil
        assert 0 <= list_no < oil.lists.size()
        l = oil.lists.at(list_no)
        with open(oil.filename, 'rb') as f:
            f.seek(l.offset + l.capacity * oil.code_size)
            return f.read(l.size * 8)


class TestPickle(unittest.TestCase):

    def dump_load_factory(self, fs):
        xq = hypervec.randn((25, 10), 123)
        xb = hypervec.randn((25, 10), 124)

        index = hypervec.IndexFactory(10, fs)
        index.Train(xb)
        index.Add(xb)
        Dref, Iref = index.Search(xq, 4)

        buf = io.BytesIO()
        pickle.dump(index, buf)
        buf.seek(0)
        index2 = pickle.load(buf)

        Dnew, Inew = index2.Search(xq, 4)

        np.testing.assert_array_equal(Iref, Inew)
        np.testing.assert_array_equal(Dref, Dnew)

        # Verify deserialized index is serializable again
        buf2 = io.BytesIO()
        pickle.dump(index2, buf2)
        buf2.seek(0)
        index3 = pickle.load(buf2)
        Dnew3, Inew3 = index3.Search(xq, 4)
        np.testing.assert_array_equal(Iref, Inew3)
        np.testing.assert_array_equal(Dref, Dnew3)

    def test_flat(self):
        self.dump_load_factory("Flat")

    def test_hnsw(self):
        self.dump_load_factory("HNSW32")

    def test_ivf(self):
        self.dump_load_factory("IVF5,Flat")


class Test_IO_VectorTransform(unittest.TestCase):
    """
    test write_VectorTransform using IOWriter Pointer
    and read_VectorTransform using file name
    """
    def test_write_vector_transform(self):
        d, n = 32, 1000
        x = np.random.uniform(size=(n, d)).astype('float32')
        quantizer = hypervec.IndexFlatL2(d)
        index = hypervec.IndexIVFSpectralHash(quantizer, d, n, 8, 1.0)
        index.Train(x)
        index.Add(x)
        fd, fname = tempfile.mkstemp()
        os.close(fd)
        try:

            writer = hypervec.FileIOWriter(fname)
            hypervec.write_VectorTransform(index.vt, writer)
            del writer

            vt = hypervec.read_VectorTransform(fname)

            assert vt.d_in == index.vt.d_in
            assert vt.d_out == index.vt.d_out
            assert vt.is_trained

            # Verify deserialized VectorTransform is serializable again
            fd2, fname2 = tempfile.mkstemp()
            os.close(fd2)
            try:
                hypervec.write_VectorTransform(vt, fname2)
                vt2 = hypervec.read_VectorTransform(fname2)
                assert vt2.d_in == index.vt.d_in
                assert vt2.d_out == index.vt.d_out
                assert vt2.is_trained
            finally:
                if os.path.exists(fname2):
                    os.unlink(fname2)

        finally:
            if os.path.exists(fname):
                os.unlink(fname)

    """
    test write_VectorTransform using file name
    and read_VectorTransform using IOWriter Pointer
    """
    def test_read_vector_transform(self):
        d, n = 32, 1000
        x = np.random.uniform(size=(n, d)).astype('float32')
        quantizer = hypervec.IndexFlatL2(d)
        index = hypervec.IndexIVFSpectralHash(quantizer, d, n, 8, 1.0)
        index.Train(x)
        index.Add(x)
        fd, fname = tempfile.mkstemp()
        os.close(fd)
        try:

            hypervec.write_VectorTransform(index.vt, fname)

            reader = hypervec.FileIOReader(fname)
            vt = hypervec.read_VectorTransform(reader)
            del reader

            assert vt.d_in == index.vt.d_in
            assert vt.d_out == index.vt.d_out
            assert vt.is_trained
        finally:
            if os.path.exists(fname):
                os.unlink(fname)


class Test_IO_PQ(unittest.TestCase):
    """
    test read and write PQ.
    """
    def test_io_pq(self):
        xt, xb, xq = get_dataset_2(d, nt, nb, nq)
        index = hypervec.IndexPQ(d, 4, 4)
        index.Train(xt)

        fd, fname = tempfile.mkstemp()
        os.close(fd)

        try:
            hypervec.write_ProductQuantizer(index.pq, fname)
            read_pq = hypervec.read_ProductQuantizer(fname)
        finally:
            if os.path.exists(fname):
                os.unlink(fname)
        self.assertEqual(index.pq.M, read_pq.M)
        self.assertEqual(index.pq.nbits, read_pq.nbits)
        self.assertEqual(index.pq.dsub, read_pq.dsub)
        self.assertEqual(index.pq.ksub, read_pq.ksub)
        np.testing.assert_array_equal(
            hypervec.vector_to_array(index.pq.centroids),
            hypervec.vector_to_array(read_pq.centroids)
        )

        # Verify deserialized PQ is serializable again
        fd2, fname2 = tempfile.mkstemp()
        os.close(fd2)
        try:
            hypervec.write_ProductQuantizer(read_pq, fname2)
            read_pq2 = hypervec.read_ProductQuantizer(fname2)
        finally:
            if os.path.exists(fname2):
                os.unlink(fname2)
        self.assertEqual(index.pq.M, read_pq2.M)
        self.assertEqual(index.pq.nbits, read_pq2.nbits)
        np.testing.assert_array_equal(
            hypervec.vector_to_array(index.pq.centroids),
            hypervec.vector_to_array(read_pq2.centroids)
        )



class Test_IO_IndexLSH(unittest.TestCase):
    """
    test read and write IndexLSH.
    """
    def test_io_lsh(self):
        xt, xb, xq = get_dataset_2(d, nt, nb, nq)
        index_lsh = hypervec.IndexLSH(d, 32, True, True)
        index_lsh.Train(xt)
        index_lsh.Add(xb)
        D, I = index_lsh.Search(xq, 10)

        fd, fname = tempfile.mkstemp()
        os.close(fd)

        try:
            hypervec.write_index(index_lsh, fname)

            reader = hypervec.BufferedIOReader(
                hypervec.FileIOReader(fname), 1234)
            read_index_lsh = hypervec.read_index(reader)
            # Delete reader to prevent [WinError 32] The process cannot
            # access the file because it is being used by another process
            del reader

            self.assertEqual(index_lsh.d, read_index_lsh.d)
            np.testing.assert_array_equal(
                hypervec.vector_to_array(index_lsh.codes),
                hypervec.vector_to_array(read_index_lsh.codes)
            )
            D_read, I_read = read_index_lsh.Search(xq, 10)

            np.testing.assert_array_equal(D, D_read)
            np.testing.assert_array_equal(I, I_read)

            # Verify deserialized index is serializable again
            data3 = hypervec.serialize_index(read_index_lsh)
            index3 = hypervec.deserialize_index(data3)
            D3, I3 = index3.Search(xq, 10)
            np.testing.assert_array_equal(D, D3)
            np.testing.assert_array_equal(I, I3)

        finally:
            if os.path.exists(fname):
                os.unlink(fname)


class Test_IO_IndexIVFSpectralHash(unittest.TestCase):
    """
    test read and write IndexIVFSpectralHash.
    """
    def test_io_ivf_spectral_hash(self):
        nlist = 1000
        xt, xb, xq = get_dataset_2(d, nt, nb, nq)
        quantizer = hypervec.IndexFlatL2(d)
        index = hypervec.IndexIVFSpectralHash(quantizer, d, nlist, 8, 1.0)
        index.Train(xt)
        index.Add(xb)
        D, I = index.Search(xq, 10)

        fd, fname = tempfile.mkstemp()
        os.close(fd)

        try:
            hypervec.write_index(index, fname)

            reader = hypervec.BufferedIOReader(
                hypervec.FileIOReader(fname), 1234)
            read_index = hypervec.read_index(reader)
            del reader

            self.assertEqual(index.d, read_index.d)
            self.assertEqual(index.nbit, read_index.nbit)
            self.assertEqual(index.period, read_index.period)
            self.assertEqual(index.threshold_type, read_index.threshold_type)

            D_read, I_read = read_index.Search(xq, 10)
            np.testing.assert_array_equal(D, D_read)
            np.testing.assert_array_equal(I, I_read)

            # Verify deserialized index is serializable again
            data3 = hypervec.serialize_index(read_index)
            index3 = hypervec.deserialize_index(data3)
            D3, I3 = index3.Search(xq, 10)
            np.testing.assert_array_equal(D, D3)
            np.testing.assert_array_equal(I, I3)

        finally:
            if os.path.exists(fname):
                os.unlink(fname)


class TestIVFPQRead(unittest.TestCase):
    def test_reader(self):
        d, n = 32, 1000
        xq = np.random.uniform(size=(n, d)).astype('float32')
        xb = np.random.uniform(size=(n, d)).astype('float32')

        index = hypervec.IndexFactory(32, "IVF32,PQ16np", hypervec.kMetricL2)
        index.Train(xb)
        index.Add(xb)
        fd, fname = tempfile.mkstemp()
        os.close(fd)

        try:
            hypervec.write_index(index, fname)

            index_a = hypervec.read_index(fname)
            index_b = hypervec.read_index(
                fname, hypervec.IO_FLAG_SKIP_PRECOMPUTE_TABLE)

            Da, Ia = index_a.Search(xq, 10)
            Db, Ib = index_b.Search(xq, 10)
            np.testing.assert_array_equal(Ia, Ib)
            np.testing.assert_almost_equal(Da, Db, decimal=5)

            codes_a = index_a.SaEncode(xq)
            codes_b = index_b.SaEncode(xq)
            np.testing.assert_array_equal(codes_a, codes_b)

            # Verify deserialized indexes are serializable again
            data3 = hypervec.serialize_index(index_a)
            index3 = hypervec.deserialize_index(data3)
            D3, I3 = index3.Search(xq, 10)
            np.testing.assert_array_equal(Da, D3)
            np.testing.assert_array_equal(Ia, I3)

        finally:
            if os.path.exists(fname):
                os.unlink(fname)


class TestIOFlatMMap(unittest.TestCase):
    @unittest.skipIf(
        platform.system() not in ["Windows", "Linux"],
        "supported OSes only"
    )
    def test_mmap(self):
        xt, xb, xq = get_dataset_2(32, 0, 100, 50)
        index = hypervec.IndexFactory(32, "SQfp16", hypervec.kMetricL2)
        # does not need training
        index.Add(xb)
        Dref, Iref = index.Search(xq, 10)

        fd, fname = tempfile.mkstemp()
        os.close(fd)

        index2 = None
        try:
            hypervec.write_index(index, fname)
            index2 = hypervec.read_index(fname, hypervec.IO_FLAG_MMAP_IFC)
            Dnew, Inew = index2.Search(xq, 10)
            np.testing.assert_array_equal(Iref, Inew)
            np.testing.assert_array_equal(Dref, Dnew)
        finally:
            del index2

            if os.path.exists(fname):
                # skip the error. On Windows, index2 holds the handle file,
                #   so it cannot be ensured that the file can be deleted
                #   unless index2 is collected by a GC
                try:
                    os.unlink(fname)
                except:
                    pass

    def test_zerocopy(self):
        xt, xb, xq = get_dataset_2(32, 0, 100, 50)
        index = hypervec.IndexFactory(32, "SQfp16", hypervec.kMetricL2)
        # does not need training
        index.Add(xb)
        Dref, Iref = index.Search(xq, 10)

        serialized_index = hypervec.serialize_index(index)
        reader = hypervec.ZeroCopyIOReader(
            hypervec.swig_ptr(serialized_index), serialized_index.size)
        index2 = hypervec.read_index(reader)
        Dnew, Inew = index2.Search(xq, 10)
        np.testing.assert_array_equal(Iref, Inew)
        np.testing.assert_array_equal(Dref, Dnew)

        # Verify deserialized index is serializable again
        data3 = hypervec.serialize_index(index2)
        index3 = hypervec.deserialize_index(data3)
        Dnew3, Inew3 = index3.Search(xq, 10)
        np.testing.assert_array_equal(Iref, Inew3)
        np.testing.assert_array_equal(Dref, Dnew3)


class TestIORoundTrip(unittest.TestCase):
    """Round-trip serialize/deserialize tests covering index types
    and sub-objects not covered by other Python tests."""

    def test_index_pq(self):
        """IndexPQ (full index, not just the PQ component)."""
        xt, xb, xq = get_dataset_2(d, nt, nb, nq)
        index = hypervec.IndexFactory(d, "PQ4np")
        index.Train(xt)
        index.Add(xb)
        Dref, Iref = index.Search(xq, 5)

        index2 = hypervec.deserialize_index(hypervec.serialize_index(index))
        D2, I2 = index2.Search(xq, 5)
        np.testing.assert_array_equal(Iref, I2)
        np.testing.assert_array_equal(Dref, D2)

    def test_index_sq8(self):
        """IndexScalarQuantizer with SQ8."""
        xt, xb, xq = get_dataset_2(d, nt, nb, nq)
        index = hypervec.IndexFactory(d, "SQ8")
        index.Train(xt)
        index.Add(xb)
        Dref, Iref = index.Search(xq, 5)

        index2 = hypervec.deserialize_index(hypervec.serialize_index(index))
        D2, I2 = index2.Search(xq, 5)
        np.testing.assert_array_equal(Iref, I2)
        np.testing.assert_array_equal(Dref, D2)

    def test_vector_transform_pca(self):
        """PCAMatrix VectorTransform with output fidelity check."""
        xt, _, xq = get_dataset_2(d, nt, nb, nq)
        pca = hypervec.PCAMatrix(d, 16)
        pca.Train(xt)

        writer = hypervec.VectorIOWriter()
        hypervec.write_VectorTransform(pca, writer)

        reader = hypervec.VectorIOReader()
        hypervec.copy_array_to_vector(
            np.array(hypervec.vector_to_array(writer.data)), reader.data)
        pca2 = hypervec.read_VectorTransform(reader)

        self.assertEqual(pca2.d_in, d)
        self.assertEqual(pca2.d_out, 16)

        ref = pca.apply(xq)
        out = pca2.apply(xq)
        np.testing.assert_array_equal(ref, out)

    def test_vector_transform_hadamard_rotation(self):
        """HadamardRotation VectorTransform write/read round-trip."""
        xt, _, xq = get_dataset_2(d, nt, nb, nq)
        fr = hypervec.HadamardRotation(d, 42)

        writer = hypervec.VectorIOWriter()
        hypervec.write_VectorTransform(fr, writer)

        reader = hypervec.VectorIOReader()
        hypervec.copy_array_to_vector(
            np.array(hypervec.vector_to_array(writer.data)), reader.data)
        fr2 = hypervec.read_VectorTransform(reader)

        self.assertEqual(fr2.d_in, d)
        self.assertEqual(fr2.d_out, d)

        ref = fr.apply(xq)
        out = fr2.apply(xq)
        np.testing.assert_array_equal(ref, out)

    def test_index_pretransform_hadamard_rotation(self):
        """Full index with HadamardRotation pre-transform round-trip."""
        xt, xb, xq = get_dataset_2(d, nt, nb, nq)
        index = hypervec.IndexFactory(d, "HR,Flat")
        index.Train(xt)
        index.Add(xb)
        Dref, Iref = index.Search(xq, 5)

        index2 = hypervec.deserialize_index(hypervec.serialize_index(index))
        D2, I2 = index2.Search(xq, 5)
        np.testing.assert_array_equal(Iref, I2)
        np.testing.assert_array_equal(Dref, D2)

    def test_null_index(self):
        """Serializing None / null index round-trips to None."""
        writer = hypervec.VectorIOWriter()
        hypervec.write_index(None, writer)

        reader = hypervec.VectorIOReader()
        hypervec.copy_array_to_vector(
            np.array(hypervec.vector_to_array(writer.data)), reader.data)
        index2 = hypervec.read_index(reader)

        self.assertIsNone(index2)
