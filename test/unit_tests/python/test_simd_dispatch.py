...
These tests verify that HYPERVEC_SIMD_LEVEL environment variable correctly
controls which SIMD implementation is used at runtime.

NOTE: These tests only work with DD builds (HYPERVEC_ENABLE_DD).
...
class TestSIMDDispatch(unittest.TestCase):
    """Tests for SIMD dispatch via HYPERVEC_SIMD_LEVEL environment variable."""
    ...
    def test_dispatch_function_exists(self):
        ...
        try:
            ...
            self.skipTest("hypervec not available")

        # Method should exist on SIMDConfig
        self.assertTrue(hasattr(hypervec.SIMDConfig, 'get_dispatched_level'))

        # Should return a value (SIMDLevel enum)
        result = hypervec.SIMDConfig.get_dispatched_level()
    ...
    def test_get_level_equals_get_dispatched_level(self):
        ...
        level = hypervec.SIMDConfig.get_level()
        dispatched = hypervec.SIMDConfig.get_dispatched_level()
        self.assertEqual(level, dispatched)

    def test_dispatch_with_env_var(self):
        """
        Test that HYPERVEC_SIMD_LEVEL controls dispatch.

        Runs hypervec in subprocesses with different SIMD levels and verifies
        that get_dispatched_level() returns the expected value.
        """
        # Only run in DD mode
        try:
            import hypervec
            if "DD" not in hypervec.get_compile_options():
        ...
        # Get the PYTHONPATH that includes hypervec module
        python_path = os.pathsep.join(sys.path)

        for level_name in levels:
            with self.subTest(level=level_name):
                # Run a subprocess with HYPERVEC_SIMD_LEVEL set
                env = os.environ.copy()
                env["HYPERVEC_SIMD_LEVEL"] = level_name
...
level = hypervec.SIMDConfig.get_level()
dispatched = hypervec.SIMDConfig.get_dispatched_level()
level_name = hypervec.SIMDConfig.get_level_name()
